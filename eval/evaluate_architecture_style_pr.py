    ...
    if args_fusion_level == 'validMapLevel':
        ...
        for k in range(n_prompt):
            ...
            for i in range(n_head):
                ...
                mask_pred = smooth(mask_pred)
                pos_vote += len(mask_pred[mask_pred==1])
                neg_vote += len(mask_pred[mask_pred==0])
            pos_dict[clip_model.positives[k]] = pos_vote
            neg_dict[clip_model.positives[k]] = neg_vote
        max_key = max(pos_dict,key=pos_dict.get)
        print(max_key)
        return max_key
...
def evaluate_postfusion(arc_keys,feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, encoder_hidden_dims, decoder_hidden_dims,
             logger,resolution,which_feature_fusion_func,is_sky_filter):
    logger.info(arc_keys)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    colormap_options = colormaps.ColormapOptions(
        colormap="turbo",
        normalize=True,
    ...
    vote_dict = {k:0 for k in arc_keys}
    for j, idx in enumerate(tqdm(eval_index_list)):
        image_name = Path(output_path) / idx
        ...
        if is_sky_filter:
            arc_keys.append("sky")
        clip_model.set_positives(list(arc_keys))

        which_fusion_func, _ = which_feature_fusion_func.split('|Loc')

        max_key = activate_stream_post(restored_feat, rgb_img, clip_model, image_name, img_ann,
        ...
        if max_key != 'sky':
            vote_dict[max_key]+=1
        logger.info(f'ID:{idx},  chosen {max_key}')
    print(vote_dict)
    logger.info(f'Final static:')
    logger.info(vote_dict)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset_name", type=str, default='wegs_VanillaCLIPExtUncetainDM-T-100epoch_catFeature12dReconUncertainlyTMAM60Kiter_trevi_fountain')
    parser.add_argument('--feat_dir', type=str, default='/home/wangyz/Documents/projects/0working/langsplat-w/output/wegs_VanillaCLIPExtUncetainDM-T-100epoch_catFeature12dReconUncertainlyTMAM60Kiter_trevi_fountain')
    parser.add_argument("--ae_ckpt_dir", type=str, default='/home/wangyz/Documents/projects/0working/langsplat-w/autoencoder/ckpt')
    parser.add_argument("--output_dir", type=str, default='/home/wangyz/Documents/projects/0working/langsplat-w/eval_result')
    parser.add_argument("--json_folder", type=str, default='/media/wangyz/DATA/UBUNTU_data/dataset/PT/label/trevi_fountain')
    ...
    parser.add_argument('--arc_keys',default=["Gothic","Baroque","Renaissance","Neoclassical"])
    ...
    output_path = os.path.join(args.output_dir, dataset_name+'arch_style_pr')
    ...
    print(args.arc_keys)
    evaluate_postfusion(args.arc_keys,feat_dir, output_path, ae_ckpt_path, json_folder, mask_thresh, args.encoder_dims, args.decoder_dims,logger,args.resolution,args.which_feature_fusion_func,args.sky_filter)
