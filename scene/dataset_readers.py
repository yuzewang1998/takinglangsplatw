...
def readNerfSyntheticInfo(path, white_background, eval, extension=""):
    ...
    scene_info = SceneInfo(point_cloud=pcd,
                           ...
                           ply_path=ply_path)
    return scene_info
def readPhotoTourismInfo(path, images, eval, llffhold=8):
    cameras_extrinsic_file = os.path.join(path, "dense/sparse", "images.bin")
    cameras_intrinsic_file = os.path.join(path, "dense/sparse", "cameras.bin")
    cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
    cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, 'dense','images'))
    # cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_name)
    # if eval:
    #     train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
    #     test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
    ...
    split_file_name = path.split('/')[-1].split('_')[0] + '.tsv'
    # open tsv file
    ...
    ply_path = os.path.join(path, "dense/sparse/points3D.ply")
    bin_path = os.path.join(path, "dense/sparse/points3D.bin")
    txt_path = os.path.join(path, "dense/sparse/points3D.txt")
    if not os.path.exists(ply_path):
        print("Converting point3d.bin to .ply, will happen only the first time you open the scene.")
        try:
            xyz, rgb, _ = read_points3D_binary(bin_path)
...
sceneLoadTypeCallbacks = {
    ...
    "PhotoTourism": readPhotoTourismInfo,
}