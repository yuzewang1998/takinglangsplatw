...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, num_aug_rendering : int):
        self.active_sh_degree = 0
        self.max_sh_degree = sh_degree
        self.num_aug_rendering = num_aug_rendering
    ...
    def decode_512_clip_feature_per_gaussian(self,ae):
        # self._language_feature:124915,12(4*3)
        batch_language_feature = self._language_feature.view(-1,3)
        batch_decoded_language_feature = decoded_language_feature = ae.decode(batch_language_feature)
        self.clip_feature = batch_decoded_language_feature.view(self._xyz.shape[0],-1)
        #self.clip_feature:124915,2045(4*512)->124915,4(valid map) and save for post fusion
        return self.clip_feature
    @property
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            if self._language_feature is None or self._language_feature.shape[0] != self._xyz.shape[0]:
                ...
                language_feature = torch.zeros((self._xyz.shape[0], 3*(1+self.num_aug_rendering)), device="cuda") #3*(1+num_view)
                self._language_feature = nn.Parameter(language_feature.requires_grad_(True))
