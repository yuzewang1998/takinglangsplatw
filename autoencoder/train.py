...
if __name__ == '__main__':
    ...
    parser.add_argument('--fusion_uncertainly_map_func',type=str,choices=['default','direct_multiply'],default = 'default')
    args = parser.parse_args()
    print('[START] Training the Auto Encoder')
    print('Training with Feature Function: {}'.format(args.train_feature_func))
    if args.train_with_uncertainly_map:
        print('Training with uncertainly map')
    else:
        print('Training without uncertainly map')
        assert args.fusion_uncertainly_map_func == 'default', 'For testing, the fusion_uncertainly_map_func should be default'
    dataset_path = args.dataset_path
    num_epochs = args.num_epochs
    train_with_uncertainly_map = args.train_with_uncertainly_map
    train_feature_func = args.train_feature_func
    data_dir = f"{dataset_path}/language_features"
    os.makedirs(f'ckpt/{args.dataset_name}', exist_ok=True)
    train_dataset = Autoencoder_dataset(data_dir,train_with_uncertainly_map,train_feature_func,is_training=True)
    ...
    for epoch in tqdm(range(num_epochs)):
        model.train()
        for idx, frame in enumerate(train_loader):
            data,uncertain_probs = frame
            data = data.to("cuda:0")
            uncertain_probs = uncertain_probs.to("cuda:0")
            outputs_dim3 = model.encode(data)
            outputs = model.decode(outputs_dim3)
            if args.fusion_uncertainly_map_func == 'default':
                l2loss = l2_loss(outputs, data)
                cosloss = cos_loss(outputs, data)
            elif args.fusion_uncertainly_map_func == 'direct_multiply':
                probs = (1-uncertain_probs).detach()
                l2loss = l2_loss(outputs*probs, data*probs)
                cosloss = cos_loss(outputs*probs, data*probs)
            else:
    ...
    print('[END] Training the Auto Encoder')